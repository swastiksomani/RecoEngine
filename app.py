...
def predict():
    ...
    if request.method == 'POST':
        ...
        #user_final_rating = pd.read_csv('temp.csv', index_col=0)

        # set columns and index names
        #user_final_rating.columns.name = user_final_rating.index.name
        #user_final_rating.index.name = 'reviews_username'

        #s = recommend_it(df, items_df, df_recom, 20,username)



        user_final_rating = pd.read_pickle(r"user_rating.pkl")
        ...
        #s = recommend_it(user_final_rating, items_df, df_recom, 20,username)
        print("model")
